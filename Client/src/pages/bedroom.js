   ...
   const handleShare = (image, title, price) => {
    ...
    if (data) {
      ...
      return (
        <div>
          <Header />
          <div className='text-2xl text-center mt-10'>Bedroom Tiles</div>
          <div className='lg:my-20 lg:mx-32 my-10 mx-10 md:mx-10 md:my-20 grid grid-cols-2 lg:gap-y-16 lg:gap-x-40 md:gap-y-16 md:gap-x-20 gap-y-10 gap-x-14'>
            {data.map((Bedroom) => (
              <div key={Bedroom._id} className='lg:space-y-3 space-y-2 md:space-y-3'>
                <FiShare2 className='text-black absolute w-6 h-6 m-4 cursor-pointer' onClick={() => handleShare(`data:image/jpeg;base64,${Bedroom.image}`)} />
                <Image
                  src={`data:image/jpeg;base64,${Bedroom.image}`}
                  alt='No Image Found'
                  onClick={()=>handleShare(Bedroom.image, Bedroom.Title, Bedroom.Price)}
                  width='0'
                  height='0'
                  className='hover:rounded-3xl lg:w-[500px]  lg:h-[300px]  md:w-96 md:h-72 w-40 h-32'
                />
                <div className='text-center'>{Bedroom.Title}</div>
                <div className='text-center'>₹: {Bedroom.Price} /-</div>
              </div>
            ))}
          </div>
        <Footer1 />
